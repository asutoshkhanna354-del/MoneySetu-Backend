...
function PartialWithdrawForm({
  ...
  const maxAmt = Math.min(10000, inv.amount);
  const isAboveMax = withdrawAmt > maxAmt;
  const isBelowMin = withdrawAmt > 0 && withdrawAmt < 10;
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div>
        <label className="text-[10px] font-bold uppercase tracking-widest block mb-1.5" style={{ color: "rgba(255,255,255,0.3)" }}>
          Withdraw Amount (₹10 – ₹{maxAmt.toLocaleString("en-IN")})
        </label>
        ...
        {isBelowMin && <p className="text-red-400 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />Min ₹10</p>}
      ...
      <div className="rounded-lg p-3" style={{ background: "rgba(249,115,22,0.05)", border: "1px solid rgba(249,115,22,0.12)" }}>
        <p className="text-[10px] leading-relaxed" style={{ color: "rgba(255,255,255,0.35)" }}>
          • Amount transferred directly to your bank account (within 24h)<br />
          • Remaining principal keeps earning daily returns<br />
          • Next withdrawal allowed after 10 days
        </p>
      </div>
