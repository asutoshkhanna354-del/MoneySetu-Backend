...
export default function ReferralPage() {
  ...
  const card = (children: React.ReactNode, extra?: React.CSSProperties) => (
    <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "20px", padding: "20px", ...extra }}>
      {children}
    </div>
  );
      ...
      <div className="space-y-5 pb-6">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm mb-1" style={{ color: "rgba(255,255,255,0.45)" }}>Total Referral Earnings</p>
            <h2 className="text-4xl font-black text-white mb-1">
              ₹{loading ? "..." : (stats?.totalCommissions?.toFixed(2) || "0.00")}
            </h2>
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>{stats?.totalTeam || 0} total team members</p>
        ...
        {/* Referral Code */}
        {card(<>
          <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "rgba(255,255,255,0.3)" }}>Your Referral Code</p>
              ...
              <div key={l.level} className="flex items-center justify-between py-3" style={{ borderBottom: l.level < 3 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>{l.pct} on each deposit</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-black" style={{ color: l.color }}>₹{l.earned.toFixed(2)}</p>
                  <p className="text-[10px]" style={{ color: "rgba(255,255,255,0.25)" }}>earned</p>
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={c.id} className="flex items-center justify-between py-3" style={{ borderBottom: i < stats.recentCommissions.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px]" style={{ color: "rgba(255,255,255,0.3)" }}>From ₹{parseFloat(c.sourceAmount.toString()).toLocaleString("en-IN")} deposit</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-black text-green-400">+₹{parseFloat(c.amount.toString()).toFixed(2)}</p>
                  <p className="text-[10px]" style={{ color: "rgba(255,255,255,0.25)" }}>{format(new Date(c.createdAt), "MMM d")}</p>
                </div>
              </div>
            ))}
          </div>
